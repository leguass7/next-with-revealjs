...
export default function (revealElement, options) {
  ...
  function cueAutoSlide() {
    ...
    if (currentSlide && config.autoSlide !== false) {
      ...
      if (
        ...
        !isPaused() &&
        !overview.isActive() &&
        (!isLastSlide() || fragments.availableRoutes().next || config.loop === true)
      ) {
   ...
   * @param {object} [event]
   */
  function onPageVisibilityChange(_event) {
   ...
   * @param {object} [event]
   */
  function onAutoSlidePlayerClick(_event) {
    // Replay
