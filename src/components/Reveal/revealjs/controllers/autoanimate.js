...
export default class AutoAnimate {
  ...
  run(fromSlide, toSlide) {
    ...
    if (fromSlide.hasAttribute('data-auto-animate') && toSlide.hasAttribute('data-auto-animate')) {
      ...
      animationOptions.slideDirection =
        allSlides.indexOf(toSlide) > allSlides.indexOf(fromSlide) ? 'forward' : 'backward'

      // Inject our auto-animate styles for this transition
      const css = this.getAutoAnimatableElements(fromSlide, toSlide).map(elements => {
      ...
      if (
        ...
        this.getUnmatchedAutoAnimateElements(toSlide).forEach(unmatchedElement => {
          const unmatchedOptions = this.getAutoAnimateOptions(unmatchedElement, animationOptions)
          let id = 'unmatched'
  ...
  autoAnimateElements(from, to, elementOptions, animationOptions, id) {
    ...
    const fromProps = this.getAutoAnimatableProperties('from', from, elementOptions)
    ...
    if (to.classList.contains('fragment')) {
      ...
      if (from.classList.contains('fragment')) {
        const fromFragmentStyle = (from.className.match(FRAGMENT_STYLE_REGEX) || [''])[0]
        const toFragmentStyle = (to.className.match(FRAGMENT_STYLE_REGEX) || [''])[0]
    ...
    if (elementOptions.translate !== false || elementOptions.scale !== false) {
      ...
      const translate = elementOptions.translate !== false && (delta.x !== 0 || delta.y !== 0)
      const scale = elementOptions.scale !== false && (delta.scaleX !== 0 || delta.scaleY !== 0)
  ...
  getAutoAnimateOptions(element, inheritedOptions) {
    ...
    if (element.parentNode) {
      const autoAnimatedParent = closest(element.parentNode, '[data-auto-animate-target]')
